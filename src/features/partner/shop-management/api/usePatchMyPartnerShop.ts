...
import { PARTNER_SHOPS_QUERY_KEY } from "@/features/partner/shop-management/api/useMyPartnerShops.ts";

export type PartnerShopPatch = {
...
export function usePatchMyPartnerShop() {
    const queryClient = useQueryClient();
    const { getErrorMessage } = useApiError();

    return useMutation({
        mutationFn: async (input: PartnerShopPatch): Promise<ShopDetail> => {
        ...
        onSuccess: (updatedShop) => {
            queryClient.setQueryData<ShopDetail[]>(PARTNER_SHOPS_QUERY_KEY, (shops) =>
                shops?.map((shop) => (shop.shopId === updatedShop.shopId ? updatedShop : shop)),
            );
        },
        onError: (error) => {
            console.error("[usePatchMyPartnerShop]", error);
