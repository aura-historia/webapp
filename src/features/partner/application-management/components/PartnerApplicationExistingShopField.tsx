...
interface PartnerApplicationExistingShopFieldProps {
    ...
    readonly selectedShop: PartnerApplicationShopSearchItem | null;
    readonly onChange: (shop: PartnerApplicationShopSearchItem | null) => void;
...
export function PartnerApplicationExistingShopField({
    ...
    const { data: shops = [], isPending } = usePartnerApplicationShopSearch(
        debouncedSearch,
    ...
    const handleSelect = (shop: PartnerApplicationShopSearchItem) => {
        onChange(shop);
        setSearch("");
        setDebouncedSearch("");
    ...
    return (
        <div className="grid gap-2">
            <Label htmlFor={id} className="gap-0">
                {t("partnerApplications.create.fields.shopId")}
                <RequiredFieldMarker />
            </Label>
            <div className="relative">
                ...
                <Input
                    ...
                    placeholder={t("partnerApplications.create.shopSearch.placeholder")}
                    className="pl-9"
                ...
                <div className="flex items-center justify-between gap-3 rounded-none border border-outline-variant bg-muted/30 px-3 py-2">
                    ...
                    >
                        ...
                        <span className="sr-only">
                            {t("partnerApplications.create.shopSearch.clear")}
                        </span>
                    </Button>
                </div>
            ) : null}
                ...
                <div className="overflow-hidden rounded-none border border-outline-variant">
                    {isPending ? (
                        <div className="flex items-center justify-center gap-2 px-3 py-6 text-sm text-muted-foreground">
                            ...
                            {t("partnerApplications.create.shopSearch.loading")}
                        </div>
                    ) : availableShops.length === 0 ? (
                        <p className="px-3 py-6 text-center text-sm text-muted-foreground">
                            {t("partnerApplications.create.shopSearch.empty")}
                        </p>
                    ) : (
