...
export function usePartnerApplicationShopSearch(search: string, enabled: boolean = true) {
    const { getErrorMessage } = useApiError();
    const normalizedSearch = search.trim();

    return useQuery<PartnerApplicationShopSearchItem[]>({
        queryKey: ["partner-applications", "shop-search", normalizedSearch],
        queryFn: async () => {
