...
type CombinationProductGridProps = {
    readonly categoryId: string;
    readonly periodId: string;
};

export function CombinationProductGrid({ categoryId, periodId }: CombinationProductGridProps) {
    const { ref, inView } = useInView();
    const { t } = useTranslation();
    const { data, isPending, error, fetchNextPage, hasNextPage, isFetchingNextPage } =
        useCombinationProducts(categoryId, periodId);
        ...
        return (
            <div className="flex flex-col items-center gap-4 py-16">
                ...
                <div className="text-center space-y-2">
                    <H3>{t("combination.products.error.title")}</H3>
                    <p className="text-base text-muted-foreground">
                        {t("combination.products.error.description")}
                    </p>
                </div>
            </div>
        );
    ...
    if (allProducts.length === 0) {
        return (
            <div className="flex flex-col items-center gap-4 py-16">
                <SearchX className="h-16 w-16 text-muted-foreground" />
                <div className="text-center space-y-2">
                    <H3>{t("combination.products.noResults.title")}</H3>
                    <p className="text-base text-muted-foreground">
                        {t("combination.products.noResults.description")}
                    </p>
                </div>
            </div>
        );
        ...
        <div className="flex flex-col gap-4">
            ...
            {(isFetchingNextPage || !hasNextPage) && (
                <ListLoaderRow
                    ...
                    loadingMoreKey="combination.products.loadingMore"
                    allLoadedKey="combination.products.allLoaded"
                />
            )}
