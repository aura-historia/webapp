...
test.describe("Imprint Page", () => {
    ...
    test("should display imprint title", async ({ page }) => {
        ...
        const title = page
            .locator("h1")
            .filter({
                hasText: /Imprint|Impressum|Mentions légales|Aviso legal/i,
            });
    ...
    test("should display service provider information", async ({ page }) => {
        ...
        const serviceProvider = page.locator(
            "text=/Service Provider|Diensteanbieter|Fournisseur du Service|Proveedor del Servicio/i",
        );
    ...
    test("should render markdown content with proper structure", async ({
        page,
    }) => {
        await page.goto("/imprint");
    ...
    test("should display legal sections", async ({ page }) => {
        ...
        const copyright = page.locator(
            "text=/Copyright|Urheberrecht|Droit d'Auteur|Derechos de Autor/i",
        );
        await expect(copyright).toBeVisible();

        // Check for applicable law section in any language
        const applicableLaw = page.locator(
            "text=/Applicable Law|Anwendbares Recht|Droit Applicable|Legislación Aplicable/i",
        );
